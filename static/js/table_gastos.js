...
editarButtons.forEach(function(button) {
    button.addEventListener('click', function() {
        ...
        window.location.href = '/editar_gasto/' + id;
    });
});
...
dataTable=$('#datatable_gastos').DataTable(dataTableOptions);
...
const listGastos = async () => {
    try {
        const response = await fetch('http://127.0.0.1:8000/list_gasto/');
            ...
            data.transacciones.forEach(transaccion => {
                content += `
                    <tr>
                        ...
                        <td>${transaccion.fk_categoria__nombre_categoria}</td>
                        <td>
