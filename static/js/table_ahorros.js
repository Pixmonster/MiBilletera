...
editarButtons.forEach(function(button) {
    button.addEventListener('click', function() {
        ...
        window.location.href = '/editar_ahorro/' + id;
    });
});
...
const dataTableOptions = {
    ...
    language: {
        ...
        zeroRecords: "Ningún Ahorro encontrado",
        info: "Mostrando de _START_ a _END_ de un total de _TOTAL_ registros",
        infoEmpty: "Ningún Ahorro encontrado",
...
dataTable = $('#datatable_ahorros').DataTable(dataTableOptions);

dataTableIsInitialized = true;
}

const listAhorros = async () => {
    try {
        const response = await fetch('http://127.0.0.1:8000/list_ahorro/');
            ...
            data.ahorros.forEach(ahorro => {
                content += `
                    <tr>
                        ...
                        <td>${ahorro.monto}</td>
                        <td>
                            <a href="${ahorro.url_edicion}" type="button" class="btn btn-primary">Actualizar</a>
                            <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#staticBackdrop">Borrar</button>
                        </td>
                    </tr>
                `;
            });
            tablebody_ahorros.innerHTML = content;
        } else {
